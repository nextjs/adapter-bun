...
class CacheHandler {
        ...
        if (softTags.length > 0) {
            ...
            if (tagEntries) {
                for (const tag of softTags) {
                    ...
                    if (tagEntry.expiredAt !== undefined &&
                        tagEntry.expiredAt > row.createdAt) {
                        return undefined;
                    }
                    if (tagEntry.staleAt !== undefined &&
    ...
    async set(cacheKey, pendingEntry) {
        ...
        store.set(cacheKey, {
            ...
            revalidateAt: entry.revalidate > 0 ? (entry.timestamp || now) + entry.revalidate * 1000 : null,
            expiresAt: entry.expire > 0 ? (entry.timestamp || now) + entry.expire * 1000 : null,
        });
    }
    async refreshTags() {
    ...
    async updateTags(tags, durations) {
        ...
        if (durations?.expire !== undefined) {
            await syncNextTagManifest(tags, {
                mode: 'stale',
                now,
